...
def generate_header_html(
    model, model_twitter, char, content_title, event_name, event_date
):
    header_html = f"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate HTML for image files with CDN paths."
    )
    parser.add_argument(
        "imgtitle", help="The title to be used for the images (Must english)"
    )
    ...
    parser.add_argument(
        "--cdn",
        type=str,
        default="https://chaos.alicey.dev/share/",
        help="CDN url (default: https://chaos.alicey.dev/share/)",
    )
    parser.add_argument(
        "--max_width",
        type=int,
        default=480,
        help="The maximum width for portrait images (default: 480px)",
    )
    parser.add_argument(
        "--max_height",
        type=int,
        default=720,
        help="The maximum height for portrait images (default: 720px)",
    )
    parser.add_argument(
        "--max_width_landscape",
        type=int,
        default=720,
        help="The maximum width for landscape images (default: 720px)",
    )
    parser.add_argument(
        "--max_height_landscape",
        type=int,
        default=480,
        help="The maximum height for landscape images (default: 480px)",
    )
    # header
    parser.add_argument(
        "--model", type=str, default=None, help="The model name (ex: すずら)"
    )
    parser.add_argument(
        "--model_twitter",
        type=str,
        default=None,
        help="The model name for Twitter (ex: suzuran_ro)",
    )
    parser.add_argument(
        "--char", type=str, default=None, help="The character name (ex: 常磐華乃)"
    )
    parser.add_argument(
        "--content_title",
        type=str,
        default=None,
        help="The content title (ex: ハミダシクリエイティブ)",
    )
    parser.add_argument(
        "--event_name",
        type=str,
        default=None,
        help="The event name (ex: AnimeJapan2025)",
    )
    parser.add_argument(
        "--event_date", type=str, default=None, help="The event date (ex: 20250322)"
    )
    # footer
    parser.add_argument(
        "--camera", type=str, default="ILCE-7M4", help="The camera name (ex: ILCE-7M4)"
    )
    parser.add_argument(
        "--lenz",
        type=str,
        default="SIGMA2470DGDNART",
        help="The lens name (ex: SIGMA2470DGDNART)",
    )
    ...
    if (
        ...
        and args.model_twitter is not None
        and args.char is not None
        and args.content_title is not None
        and args.event_name is not None
        and args.event_date is not None
    ):
        r = generate_header_html(
            args.model,
            args.model_twitter,
            args.char,
            args.content_title,
